...
#set opts
# set desktop session
DESKTOP_SESSION="${DESKTOP_SESSION:-dwm}"
# set config dir
...
# @File          : autostart.sh
# @Description   : autostart script for dwm
# @TODO          :
# @Other         :
# @Resource      :
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
# Help
if [[ "$1" = *help ]]; then
  printf "\n\t\t%s\n" "Usage: $PROG" "Starts applications for dwm window manager"
  exit
fi
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
if __desktop_name "awesome" || __desktop_name "dwm" || __desktop_name "i3" || __desktop_name "qtile" || __desktop_name "sway" || __desktop_name "xmonad"; then
  true
else
