...
# @File          : dwm
# @Description   : Installer script for dwm
# @TODO          :
# @Other         :
# @Resource      :
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# end with a space
APP="$APPNAME conky notify-send iwconfig dmenu sxhkd "
PERL=""
PYTH=""
PIPS=""
CPAN=""
...
run_postinst() {
  desktopmgr_run_post
  mkdir -p "$APPDIR/src"
  cp -Rf "$INSTDIR/src/." "$APPDIR/src/"
  ln -sf "$APPDIR/src/config.def.h" "$APPDIR/config.h"
  if cd "$INSTDIR/src"; then
    make &>/dev/null &&
